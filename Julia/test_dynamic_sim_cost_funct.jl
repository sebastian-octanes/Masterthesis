...
for i in lin
    ...
    while isopen(window)
        ...
        if(steps >= 40)
            print(avg_time)
            avg_time =  vcat(avg_time, time_total / (steps + 20))
...
open("outputFiles/costFunctCompTimeSoftConstLin.dat", "w") do io
    writedlm(io, [lin avg_time avg_time_without_init])
end
