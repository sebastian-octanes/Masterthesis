...
Für das dynamische Fahrzeugmodell wurde eine minimale Rundenzeit 4.45 s gemessen.  Die damit erhaltene maximale Querbeschleunigung ist nur fünf Prozent größer als die des kinematischen Models. Zum Vergleich, die schnellste gefahrene Runde im Deutschen Event der Formula Student waren 5.7 Sekunden.
...
Bei einer rein longitudinalen oder konstanten lateralen Bewegung ist, wie im vorherigen Kapitel schon gezeigt wurde, die Differenz der einzelnen Fahrzeugmodelle nicht groß. Bei dynamischeren Szenarien werden die Unterschiede dahingegen deutlich sichtbar. In der Darstellung \ref{fig:modelDiffOrient} ist für ein oszillierenden Lenkwinkel die Änderung der Orientierung veranschaulicht. Zuerst wird die Bewegung ohne jegliche longitudinale Beschleunigung betrachtet und anschließend mit maximaler möglicher Beschleunigung .
...
\end{figure}
Der Unterschied welcher sofort ins Auge fällt ist die zeitliche Differenz, in dem das kinematische und das dynamische Modell die maximale Orientierung erreichen.
Die Differenz ist auf das Rotationsmoment des Fahrzeugs zurückzuführen, welches im rein geometrischen Modell nicht beachtet wird. Es führt dazu, dass nach dem erreichen der Nullstellung des Lenkwinkels das Fahrzeug noch einen kurzen Moment weiter dreht. Dies ist auch der Grund, warum der Spitzenwert der Orientierungen unterschiedlich sind.  
Die Differenz zwischen dem dynamischen und mit Kamschen Kreis erweiterte Fahrzeugmodell ist bei nicht vorhandener Beschleunigung nur gering.  
...
Dies ändert sich, sobald eine zusätzliche longitudinale Kraft durch den Motor erzeugt wird. Diese Situation ist in Abbildung \ref{fig:modelDiffOrientMaxAcc} aufgezeigt. Der erste Punkt welcher ins Auge fällt, ist die nicht vorhandene Änderung zwischen kinematischem und dynamischem Modell (welches nicht durch den Kammschen Kreis erweitert ist). Da die Kräfte nicht eingeschränkt werden, sind beide Kurven vom Verlauf gleich dem Fall ohne Beschleunigung. Die Trajektorie des um den Kammschen Kreis erweiterte Modell unterscheidet sich nun jedoch signifikant. Die Kraft welche lateral wirkt, wird durch die longitudinale Kraft eingeschränkt und verringert daher das rotatorische Moment. Diese Situation ist jedem bekannt, der in seiner Freizeit Kart fährt. Das Einbremsen in eine Kurve geschieht meist bei fast gerade Lenkradstellung und in der Kurve direkt wird kaum gebremst um ein wegrutschen zu verhindern.

\subsection{Position}

Um den Einfluss der verschiedenen Modelle nochmals zu visualisieren, wird zusätzlich die gefahrene Trajektorie betrachtet. In der Abbildung \ref{fig:modelDiffPos} [links] sind diese sowohl für 5 $\frac{m}{s}$ als auch 30 $\frac{m}{s}$ dargestellt. 

\subsection{Reifenmodelle}
Das Reifenmodell welches bis zu diesem Punkt in der Arbeit verwendet wurde entspricht dem genauen \emph{Magic Model}. Im folgenden soll nun kurz untersucht werden, was für einen Unterschied bezüglich der Rechenzeit die drei verschiedenen Modelle haben und wie groß deren Auswirkung auf das Verhalten des Fahrzeugs ist.



\subsection{Longitudinale Modelle}


\section{Regelung des dynamischen Modells im Rennkurs}
In der Realität ist das kinematische Modell nur für geringe Geschwindigkeiten genau genug die Bewegung des Fahrzeugs zu Modellieren. Im Folgenden wird deswegen das Modell im Simulator durch ein dynamische Fahrzeugmodell mit berechneten Reifenkräften ersetzt. Das Ziel ist zu evaluieren, wie gut sich das deutlich komplexere System mit dem weiterhin sehr einfachen kinematischen Modell im \ac{MPC}-Algorithmus regeln lässt.
Zuerst wird untersucht, welchen Einfluss die Geschwindigkeit und Beschleunigung auf die drei Modelle, kinematisch, dynamisch und dynamisch mit Kammschen Kreis haben. 
Danach werden die einzelnen elastischen Begrenzung auf ihre Fähigkeit getestet das dynamische Modell auf dem Kurs zu halten und welche Rundenzeiten sie ermöglichen. Im letzten Schritt wird zusätzlich noch untersucht welchen Einfluss $\beta_{max}$ auf die Regelfähigkeit des Algorithmus hat und ob sie sich durch verändern der Werte verbessern lässt. 
