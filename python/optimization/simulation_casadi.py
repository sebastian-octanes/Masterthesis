...
class SimulationEnvironment():
    ...
    def init_mpc(self, N):
        ...
        self.x = SX.sym("x",(N+1) *6)
        print "\n state vector: ", self.x
        
        self.G = vertcat(self.x[0], self.x[1], self.x[2], self.x[3])
        self.G = self.vehicleModel.vehicle_model_cassadi_(self.x, self.G, N)
        print "\n vehicle model constraints" ,self.G
        #set the track bounds constraint
        init_state = [0, 0, 0.01, math.pi/2, 0, 0]
        self.init_state_vector(N, init_state)
        self.constraints = Constraints(init_state, self.vehicleModel, self.raceTrack)
        self.constraints.ineq_constraint_vehicle_bounds_set_tangent_points(self.X0)
        self.G = self.constraints.ineq_const_track_bounds_casadi_(self.x, self.G, N)
        print "\n bound constraints: ", self.G
        #set cost function
        self.J = 1/self.x[2]
        for i in range(N):
            self.J = self.J + 1/self.x[(i+1)* 6 + 2]
     
        print "\n Cost function: ", self.J
        print self.x
        #set initial state to init_m to mark starting point
        self.nlp = {'x':self.x, 'f':self.J, 'g': self.G}

        # Allocate an NLP solver
        self.opts = {"ipopt.tol":1e-7, "ipopt.print_level": 3}
        self.solver = nlpsol("solver", "ipopt", self.nlp, self.opts)        
        
        # Bounds on u and initial condition
        lbg = np.array(init_state[0:4])
        ubg = np.array(init_state[0:4])

        lbx = np.array([-inf, -inf, 0.001, -inf, - self.vehicleModel.max_long_dec, -self.vehicleModel.max_steering_angle])
        ubx = np.array([ inf,  inf, self.vehicleModel.max_speed,  inf, self.vehicleModel.max_long_acc,  self.vehicleModel.max_steering_angle])
        ...
        for i in range(N):
            ...
            lbx = np.append(lbx, np.array([-inf, -inf, 0.001, -inf, - self.vehicleModel.max_long_dec, -self.vehicleModel.max_steering_angle]))
            ubx = np.append(ubx, np.array([ inf,  inf, self.vehicleModel.max_speed,  inf, self.vehicleModel.max_long_acc,  self.vehicleModel.max_steering_angle]))
        #add ineq 
        for i in range(N):
        ...
        self.arg["ubg"] = ubg
        print "G: ", self.G
        print "lbg: ", self.arg["lbg"]
        print "ubg: ", self.arg["ubg"]
        self.arg["x0" ] = x0



        #this is for tracking simulation time dont delete
        self.computation_time = pygame.time.get_ticks()
        self.simulation_time = pygame.time.get_ticks()
        self.simulation_steps = 1
    
    
    def create_new_mpc(self):
        self.x = SX.sym("x",(self.N+1) *6)
        
        self.G = vertcat(self.x[0], self.x[1], self.x[2], self.x[3])
        self.G = self.vehicleModel.vehicle_model_cassadi_(self.x, self.G, self.N)
        #set the track bounds constraint
        init_state = self.X0[0:6]
        #self.constraints.ineq_constraint_vehicle_bounds_set_tangent_points(self.X0)

        self.G = self.constraints.ineq_const_track_bounds_casadi_(self.x, self.G, self.N)
        #print "\n bound constraints: ", self.G
        #set cost function
        self.J = 1/self.x[2]
        ...
        # Allocate an NLP solver
        self.opts = {"ipopt.tol":1e-15, "ipopt.print_level": 6}
        self.solver = nlpsol("solver", "ipopt", self.nlp, self.opts)        
    ...
    def simulate(self):
        while not self.loopSimDone:
            ...
            if(not self.mpcActive):
                ...
                self.X0[0:4] = self.vehicleModel.compute_next_state_(res[0:6])
                ...
                #self.G = self.constraints.ineq_const_track_casadi_update_points(self.x, self.G, self.N)
                #self.nlp = {'x':self.x, 'f':self.J, 'g': self.G}                
                #self.solver = nlpsol("solver", "ipopt", self.nlp, self.opts)        
                self.create_new_mpc()
                #set new init_state for constraint                
                self.arg["lbg"][0:4] = self.X0[0:4]   
                self.arg["ubg"][0:4] = self.X0[0:4]
                
                self.constraints.set_initial_state(self.X0[0:4])                
                self.set_car_path()
                self.display_car_info(res[4], res[5])
                self.plot_predicted_path(res)
                self.plot_track_bound_constraint(res)
